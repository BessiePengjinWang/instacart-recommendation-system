...
class TwoStageRecommender:
    ...
    Stage 1: ALS for candidate generation (recall)
    Stage 2: LightGBM for ranking (precision)
    """

    def __init__(self, ranker_model, n_candidates=500, als_factors=100):
        ...
        self.user_item_matrix = None  # Keep for recommend() calls

    def fit_als(self, prior_data, all_products=None):
        print("Training ALS for candidate generation...")

        interactions = prior_data.groupby(['user_id', 'product_id']).size().reset_index(name='count')
        ...
        if all_products is not None:
            ...
            if len(missing_products) > 0:
                print(f"  Adding {len(missing_products)} products to vocabulary (rare items)")
        ...
        print(f"  Sparsity: {1 - self.user_item_matrix.nnz / np.prod(self.user_item_matrix.shape):.6f}")
    ...
    def generate_candidates(self, user_id, n=None):
        if n is None:
            n = self.n_candidates
    ...
    def predict(self, test_df, return_candidates=False):
        print("Running two-stage prediction...")
        start_time = time.time()

        # Stage 1: Generate candidates (TRUE BATCH MODE)
        ...
        # Vectorized candidate lookup via merge instead of .apply()
        candidate_pairs = []
        ...
        # Stage 2: Rank candidates with LightGBM
    ...
    def _batch_generate_candidates(self, user_ids):
        """
        True batch candidate generation using implicit's native batch recommend.
        """
        user_candidates = {}
        ...
        # Single batch call — implicit handles multithreading internally
        ...
        # all_item_indices shape: (n_users, n_candidates)
    ...
    def evaluate_stage1_recall(self, test_df, y_true):
        print("Evaluating Stage 1 recall...")

        unique_users = test_df['user_id'].unique()
        candidate_set = self._batch_generate_candidates(unique_users)

        # Vectorized lookup via merge
        candidate_pairs = []
        for uid, pids in candidate_set.items():
            for pid in pids:
                candidate_pairs.append((uid, pid))
...
def compare_single_stage_vs_two_stage(single_stage_model, two_stage_model, test_df, y_true):
